...
if ($re !~ $specials_re) {
  ...
  while (my ($k, $v) = each %matches) {
    ...
    my $words = join '&amp;text=', map { my @rv; for my $idx (0..$#{$$_{words}}) { next unless  $$_{words}[$idx] =~ /\S/; push @rv, map {s/^(.*)[+]// and length and $_.= '-,' for $$_[0]; length $$_[3] and s/^([^%]+)%.*$/$1/ for $$_[1]; length and $_ = !/^[\+%]/ && ",-$_" and s/^([^%]+)%.*$/$1/ for $$_[2]; length and $_ = ",$_" for $$_[3]; "$$_[0]$$_[1]$$_[3]$$_[2]"} [map {s/-/%2D/g; s/^(?i:%2b|[.]){3}[+]|[+](?i:%2b|[.]){3}$//g; $_} encode($$_{pre}[$idx] !~ / \.{3}$/ ? $$_{pre}[$idx] =~ /^ *(.*)$/ && $1 : ""), encode($$_{words}[$idx]), encode($$_{pre}[$idx+1] !~ /^\.{3} / ? $$_{pre}[$idx+1] =~ /^(.*?) *$/ && $1 : ""), encode($$_{end}[$idx]//"")]; } @rv } @$v;
    $words =~ s/[+]+/%20/g;
    $words =~ s/%20(&amp;|$)/$1/g;
