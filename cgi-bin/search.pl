...
my $pffxg = run_shell_command "cd $d && timeout 5 pffxg.sh" => [qw/--no-exclusions --no-cache --unzip --html --markdown --/, qw/-P -e/], $re;
...
while (my ($k, $v) = each %matches) {
  ...
  my $uncompressed;
  gunzip $filename, \$uncompressed and utf8::decode $uncompressed and $filename = \$uncompressed if $filename =~m#\.gz[^/]*$#;
