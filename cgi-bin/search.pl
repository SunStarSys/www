...
if ($re !~ $specials_re) {
  ...
  if (-d $lucy_index) {
    ...
    while (my $hit = $hits->next) {
       ...
       push @{$matches{$path}}, {count => 1000 * $hit->get_score, match => Template("{{content|lede|safe}}")->render({content => $hit->{content}}) || Template("{{content|truncatewords:10|safe}}")->render({content => $hit->{content}}), pre => [], words => [], end => []};
    }
  }
