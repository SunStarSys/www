...
if ($re !~ $specials_re) {
  ...
  while (my ($k, $v) = each %matches) {
    ...
    push @matches, [$data{mtime}, $total, qq([<a href="./?regex=^Status:\\s$status;lang=$lang;markdown_search=1"><span class="text-warning">$status</span></a>] <a href="$link#:~:text=$words">$title</a> $rev), $k, [map {my @w = "text=$words" =~ /text=[^&]+/g; my $i; s/(<span class="text-danger">[^<]+<\/span>)/qq(<a href="$link#:~:) . $w[$i++] . qq(">$1<\/a>)/ge; $_} map $_->{match}, @$v]]
