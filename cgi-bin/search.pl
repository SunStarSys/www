...
if ($re !~ $specials_re) {
  ...
  while (my ($k, $v) = each %matches) {
    ...
    my $words = join '&amp;text=', map { my @rv; for my $idx (0..$#{$$_[0]}) { push @rv, map "$$_[0]-,$$_[1],-$$_[2]", [map {s/-/%2D/g; s/^\.{3} | \.{3}$//g; $_} encode($$_[0][$idx]), encode($$_[1][$idx]), encode($idx > 0 ? $$_[0][$idx-1] : "")]; } @rv } map [$_->{pre}, $_->{words}], @$v;
    $words =~ s/[+]+/%20/g;
    $words =~ s/%20(&amp;|$)/$1/g;
    $words =~ s/text=[^,]*,%20(?:&amp;|$)//g;
