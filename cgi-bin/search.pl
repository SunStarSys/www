  ...
  if (exists $pw{$svnuser}) {
    ...
    if ($re =~ /^friends=$/i) {
      ...
      for (@friends) {
        ...
        if ($$_{members}) {
          $graphviz .= "\"$dt\" [name=\"$dt\",fontcolor=green,URL=\"./?regex=$_->{text};lang=$lang;markdown_search=1\",tooltip=\"\"];\n" unless $seen{$dt}++;
          $graphviz .= "\"$svnuser\" -> \"$dt\" [color=green];\n";
          for my $m (@{$$_{members}}) {
            my $mdt = substr $m->{text}, 0 , -1;
            $graphviz .= "\"$mdt\" [name=\"$mdt\",fontcolor=blue,URL=\"?regex=$m->{text};lang=$lang;markdown_search=1\",tooltip=\"\"];\n" unless $seen{$mdt}++;
            $graphviz .= "\"$dt\" -> \"$mdt\";\n";
          }
        }
        elsif ($$_{groups}) {
          $graphviz .= "\$dt\" [name=\"$dt\",fontcolor=blue,URL=\"./?regex=$_->{text};lang=$lang;markdown_search=1\",tooltip=\"\"];\n" unless $seen{$dt}++;
          $graphviz .= "\"$svnuser\" -> \"$dt\" [color=green];\n";
          for my $g (@{$$_{groups}}) {
            my $gdt = substr $g->{text}, 0, -1;
            $graphviz .= "\"$gdt\" [name=\"$gdt\",fontcolor=blue,URL=\"?regex=$g->{text};lang=$lang;markdown_search=1\",tooltip=\"\"];\n" unless $seen{$gdt}++;
