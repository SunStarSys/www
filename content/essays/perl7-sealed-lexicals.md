...
This isn't a trifling issue with calls to `C struct` get-set accessor methods- the common situation with many mod_perl APIs.  Perl's runtime method-call lookup penalty on the httpd `request_rec struct` methods mod_perl provides is on the same order of magnitude of the full execution of the call.  For mod_perl backed sites making millions of XS method calls a second, this is an awful waste of precious CPU cycles.
