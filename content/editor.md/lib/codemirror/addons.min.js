...
CodeMirror.runMode = function(string, modespec, callback, options) {
  ...
  if (callback.nodeType == 1) {
    var tabSize = (options && options.tabSize) || CodeMirror.defaults.tabSize;
      var node = callback, col = 0, lineno = callback.parentElement.parentElement.parentElement.getAttribute("data-offset");
      if (! lineno)
	  lineno = 0; 
      node.innerHTML = (options && options.lineNumbers) ? "<style type='text/css'> span.line-numbers {color: #888; padding: 4px 4px; text-align: right; user-select: none;}</style>" : "";
    callback = function(text, style) {
      if (col==0 && options && options.lineNumbers) {
	  lineno++;
	  var sp = node.appendChild(document.createElement("span"));
	  sp.className="line-numbers";
	  var t = lineno + " ";
	  if (t.length == 2)
	      t = "  " + t;
	  if (t.length == 3)
