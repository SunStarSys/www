...
Совместимое с FIPS 140-2 тройное шифрование W/MFA для служб с обратной передачей портов (HTTPS/SSH/IPsec).  Пояс, подтяжки и помехи!

----

&nbsp;

### Без пароля [RBAC](https://en.wikipedia.org/wiki/Role-based_access_control) модель, перец с [орфрус](https://github.com/SunStarSys/orthrus) [Отп-sha1](https://en.wikipedia.org/wiki/One-time_password) Задачи sudo

На серверах не сохранены фиксированные пароли. Это *лимит* автономная автоматизация использования sudo / RBAC, по уважительной причине.  Однако мы имеем [инструмент](https://github.com/SunStarSys/pty) устранить труд  реагируя на подсказки различных видов.
...
Мы используем TCP-прокси, а не HTTP и бэкенды MSA, **так что единственным хостом, который видит ваш веб-трафик, является хост, который расшифровывает ваш трафик TLS**. Те же правила применяются к трафику Subversion: только прямой трафик с сквозным шифрованием TLS в конечную точку **сервиса** видит ваши данные в полете.

Удачи с уровнями **MSA и уровнями конфиденциальности данных** с другими поставщиками. Враг "нефункциональная инженерия" Это сложность. Гораздо проще обеспечить значимые обещания безопасности, когда ваш продукт является **федерированным монолитом вместо массивного MSA minfield**, который является еще одним противоположным дифференциатором между Orion и его областью конкурентов.
...
Орион's модель безопасности централизованно управляется настройками, содержащимися в [`@path::акл`](#) как в [`lib/path.pm`](#). Файлы конфигурации сервера Offshoot динамически генерируются при каждом фиксированном изменении.

### OpenIDC Защита единого входа

Сеансовые файлы cookie отмечены флагом HttpOnly и Secure, поэтому попытки кражи сеанса JavaScript эффективно нейтрализуются онлайн-редактором Orion.
...
Каждый ресурс рабочей копии, доступный через пользовательский интерфейс, проверяется на соответствие спискам контроля доступа Subversion перед их представлением пользователю.  Таким образом, мы гарантируем, что доступ для чтения к несанкционированным ресурсам для активов с контролем версий (он же **все**).
...
#### Создание сайта и Subversion ACL's синхронизировано с &#64;path::acl мгновенно при фиксации
