...
#### Разделение консенсов и инженерных компромиссов
...
В (noSQL) Орион мы рассматриваем редактирование и рендеринг как две отдельные проблемы, которые должны быть обработаны двумя независимыми программными стеками; где гораздо больше процессов, проектирования, проверки и управления зависимостями инвестируется в интерфейс редактирования. Таким образом, мы можем ограничить программное обеспечение стека рендеринга файловым сервером Apache с поддержкой SSI, включающим стандартную веб-аутентификацию и элементы управления доступом, и оптимистично ожидаем, что контент просматривается на порядок чаще, чем он редактировался.
...
На самом деле, стоимость на несколько секунд больше воздействия строительной техники, прежде чем иметь возможность просматривать опубликованные изменения на сайте.
И польза? Вы никогда не увидите, что ваш сайт снова взломан** через уязвимости нулевого дня в стеке программного обеспечения веб-сервера рендеринга.
