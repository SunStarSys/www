...
- `депс` &mdash; переопределяет обычный `fetch_deps` переработка
...
Вместо этого создайте связанные сгенерированные исходные файлы (например, `.bib\$lang` $$\mapsto$$ `\$base.page/bibliography.yml\$lang (язык)`), который должен быть построен на вторичном запуске системы сборки.
...
Утилита для последовательной обработки `$арг{просмотр}`.
...
Добавление `$арг{язык}` по `$арг{шаблон}`.
...
Синтаксический анализ headers+content файла в кодировке UTF-8 `Файл $` и хранит результаты в хэшрефе `$аут`. `$content_lines` (необязательно) максимальное число строк контента для чтения.
...
Безопасные обновления `@path::акл` для этого `$path`, исходя из его `акл` спецификация заголовка. По умолчанию используется `$_` как путь, если не пройдены аргументы.
