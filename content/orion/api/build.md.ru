...
2. условно ходить [`содержание/`](#) дерево для семян [`%path::зависимости`](#) и [`@path::acls`](#).

Задача последнего состоит в том, чтобы обеспечить вызываемый [`просмотр`](#)на основе [`$метод`](#)s для соответствующих записей в [`@path::шаблоны`](#) (как строковое имя метода во втором слоте каждой записи ссылки массива), вызвано как...
...
Сборки и кэширование [`асимптота`](#) трехкодовые блоки.
...
- [`путь`](#)
- [`данные`](#) - магазины, образующие анон-массив депсов
