...
sequenceDiagram
    participant Alice
    participant Bob
    Алиса->>Джон: Привет Джон, как Вы?
    loop Healthcheck
        Джон->>Джон: Бой против ипохондрии
    end
    Note right of John: Логическое обоснование мысли<br/>преобладать...
    John-->>Алиса: Отлично!
    Джон->>Боб: Как о тебе?
    Bob-->>Джон: Jolly Good!
```
...
gantt
    ...
    Другая задача     :после a1  , 20d
    section Another
    Задача в секунду      :2014 год-01-12  , 12d
    другая задача      : 24d
```
----
```mermaid
...
%%{init:{"theme":"default"}}%%
graph TB
    sq[Квадратная форма] --> Ци((Круг форма))

subgraph A
        od>Нечетная форма]-- Две линии<br/>комментарий к границе --> рожать
        di{Ромб с <br/> грейдер} -.-> рожать(Округление<бр>квадрат<br>форма)
        di==>ro2(Rounded квадратная форма)
    end

%% Notice that no text in shape are added here instead that is appended further down
    e --> od3>По-настоящему длинный текст с разрывом строки<br>в нечетной форме]

%% Комментарии после двойного процента знаков
    e((Внутренний / круг<br>и некоторые странные <br>специальные символы)) --> f(,.?!+-*ز)

цир[Cyrillic]-->cyr2((Circle форма));
...
mindmap
  root((ментальная карта))
    Происхождение
      ...
      ::icon(fa fa-книга)
      Популяризация
        Британский популярный автор психологии Тони Бузан
    Исследования
      По эффективности<br/>и особенности
      Вкл. Автоматическое создание
        Использование
            Творческие методы
            Стратегический планирование
            Аргумент картография
    Инструменты
      Ручка и бумага
      Русалка
```
