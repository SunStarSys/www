---
acl: '@staff=rw, *=r'
categories: ~
dependencies: '*.md.ru api/index.md.ru'
keywords: wiki,jamstack,блог,безопасно,безопасно
status: завершено
title: SunStar Орион систем
---

<div class="row">
<div class="col-lg-5">
<p>
	<span class="text-success"><img style="width:100px" src="../images/sunstar-orion-symbol-linear.png"><!-- Derived work from Dennis Moskowitz's original CC BySA v4.0 --> &nbsp;<em>Платформа Jamstack Wiki.</em></span>&trade;
</p>
<span class="text-white">SunStar Орион систем</span> &nbsp; является
<p>
	<ul>
	<li><p>

[**невзломный**](security),

</p></li>
<li><p>

[**неразрывной**](https://iconoclasts.blog/joe/apache-cms#:~:text=7%20years),

</p></li>
	<li><p>мобильный дружелюбный,</p></li>
	<li><p>

[**пиковая производительность**](features),

</p></li>
	<li><p>федеративный</p></li>
	</ul>
</p>

{# lede #}<span class="text-white">Корпоративная платформа Jamstack Wiki</span>&nbsp; для <b>создания, управления и доставки</b> статические веб-страницы с использованием технологии Jamstack для динамических функций в них{# lede #}. Для получения информации о ценах см. наш [Планы Ориона](plans).

### Вот шаг, ответственный за принятие решений...

<span class="text-white">Почему выбирают Орион? Платформа Jamstack Wiki, которая превосходит &mdash; и затраты намного меньше</span>

То, что действительно отличает Orion, не просто еще одна вики-платформа &mdash; Это наша собственная технология Orion, проверенный в бою и запатентованный подход, который обеспечивает элегантный пользовательский опыт с непревзойденной эффективностью.

Это позволяет нам выполнять сверхнизкие операционные расходы, предлагая превосходные средства контроля редакций, безопасность корпоративного уровня и новаторские функции, которые не могут соответствовать устаревшим вики-ресурсам. Мы передаем эту огромную экономию непосредственно вам. Ожидайте только скромные, прозрачные сборы за точные облачные ресурсы, которые вы используете &mdash; нет раздутых ценовых уровней или скрытых сборов.

Орион построен для лидеров, которые отказываются поселиться:

- **Лидеры университетов** &mdash; Устраните плагиат в кампусе и обеспечьте безопасную совместную работу с открытым доступом с помощью совместного использования документов, курирования и онлайн-публикации с контролем версий. Превратите управление академическими знаниями в современное, доступное для аудита, <span class="text-success">Открытая наука</span> Платформа.

- **Правительство &amp; лица, принимающие решения в области ИТ** &mdash; Соблюдайте требования к инфраструктуре FedRAMP, NIST 800-207, требующие нулевого доверия, и детализированные средства контроля доступа в репозитории управления версиями &mdash; все с безопасностью, совместимой с FIPS 140-3, и возможностью установки внутри вашего корпоративного VPN.

- **Руководители маркетинга** &mdash; Используйте быструю, глобальную, инновационную корпоративную платформу для корпоративных ресурсов веб-сайтов. Использование Orion в качестве мощной автономной CMS &mdash; безопасная, производительная и полностью отделенная от внешнего интерфейса доставка.

- **Технологические компании &amp; веб-агентства** &mdash; Замените небезопасные коммерческие вики, приручите горы документации по продуктам, клиентские сайты и даже двоичные загрузки. Отбирайте все с уверенностью, скоростью и историей версий.

- **Архитекторы высокой доступности &amp; Основные инженеры** &mdash; Избегайте одиночных точек отказа, встроенных в стеки LAMP на основе SQL. Реализуйте настоящую масштабируемость Jamstack: параллельные сборки, инкрементные обновления, глобальный охват POP и пиковая производительность, измеряемая в миллисекундах.

**Любой бизнес устал от экономики вики** &mdash; Откажитесь от дорогостоящих подписок на SaaS, хрупких бесплатных инструментов, которые истощают время персонала, и устаревших систем, которые замедляют работу вашей команды. Orion дает вам мощность предприятия за долю от общей стоимости владения (часто ниже 10 000 долларов США в год для полного использования на предприятии).

Отсутствие привязки к поставщику &mdash; И мы докажем это*. Ваш контент живет в чистых Markdown и стандартных рабочих процессах управления версиями. Нет фирменных форматов. Никаких миграционных кошмаров.

Хотите прозрачности? Убедитесь сами:

Вся логика сборки для этого веб-сайта: менее 100 строк кода, в комплекте со сложным, запоминаемым управлением зависимостями, которое автоматически генерирует многоязычные карты сайта, индексы каталогов и многое другое.

Посмотреть исходник: [наше дерево исходного библиотеки](https://vcs.sunstarsys.com/repos/svn/public/cms-sites/www.sunstarsys.com/trunk/lib/)или его [<i class="fa fa-github fa-emoji" title="Github"></i> зеркало GitHub](https://github.com/SunStarSys/www/tree/trunk/lib).

Базовый генератор статических сайтов: открытый источник на GitHub под лицензией Apache 2.0 → @SunStarSys/orion

Это не маркетинговый спин &mdash; Это проверенное, открытое инженерное наследие, начиная с инноваций F/OSS, впервые появившихся в 2011 году. Готовы испытать вики, которая на самом деле экономит время, деньги и головные боли? [Позвоните нам или напишите нам](/contact) сегодня.

</div>

<div class="col-lg-7">
	<div class="embed-responsive embed-responsive-16by9">
	 	<iframe class="embed-responsive-item" style="max-width:560;max-height:315" src="https://www.youtube.com/embed/ylFF7PDniZw" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
	</div>
	<p style="height:20px">&nbsp;</p>

<h3>Вот почему, молодой академик / Full Stack разработчик / Публичный блоггер...</h3>

Хотите сайт, который **безопасен по умолчанию**? Легко обновлять и управлять, скрывая сложность базовой системы управления версиями, но обеспечивая полную мощность, когда она действительно нужна?  Мгновенная публикация, когда это необходимо; плавная поэтапная сборка филиалов и настраиваемые детализированные рекламные акции для вашего производственного сайта, поскольку это соответствует предпочтительному рабочему процессу вашей команды.

Если вы устали от лицензирования на место и выбора на ЦП, взвешивающих вас всякий раз, когда вам нужно исправить опечатку, [связаться с нами](/contact). Если вы отвечаете за курирование контента и ищете настраиваемую базу данных фактов для управления вашими внешними ссылками, или если вы укротитель львов с регулярным выражением для Десятилетиями накопленного устаревшего HTML-контента, или просто кого-то, недовольного другими вариантами на рынке и тем, как они функционируют в масштабе, у нас есть что-то действительно особенное, чтобы предложить вам.

Никогда не иметь дело непосредственно с системой контроля версий или даже статическим генератором сайта, на вашем персональном компьютере зависит от вас. Интерактивная среда IDE разработана и готова для **рабочего процесса только для браузера** в любое время.  В крайнем случае, даже мобильный (браузерный) интерфейс сделает это *мгновенно*.

<div id="lightning">

![Молния](index.page/lightning)

</div>

<style type="text/css">

div#lightning img {width:100%}

</style>

Многогигабайтные исходные деревья просто не соответствуют им &mdash; Полная продолжительность сборки сайта измеряется в *секундах*, а не часах и минутах.  Система легко поддерживает [`300 МБ/с`](features) при обработке ваших источников для полной сборки сайта, но учитывая его <span class="text-white">уникальное, запатентованное управление зависимостью</span> функциональность, к ней практически никогда не нужно прибегать:

<span class="text-success"><em>Мы строим только то, что вам нужно.</em></span>&trade;

Это [технология](https://iconoclasts.blog/joe/apache-cms) был впервые изобретен в конце 2010 года [Пол Керна](https://paul.querna.org/articles/2010/10/22/evolution-of-apaches-websites/)Даниэль Шахаф, Дейв Фишер, и [Джо Шефер](https://iconoclasts.blog/joe/) как члены команды инфраструктуры Apache. Фраза "JAMstack" был изобретен много лет спустя (2015), группой, не знавшей о нашем предыдущем искусстве в этом пространстве.  Но [общественная запись](http://web.archive.org/web/20120112045033/http://www.apache.org/dev/cms) кристально ясно, и это новое предложение от SunStar Systems является свидетельством этого оригинального наследия и его истинного происхождения с открытым исходным кодом.

<h4>Хотите, чтобы он быстро вращался, чтобы увидеть, насколько он быстрый и легкий?</h4>

Этот веб-сайт самостоятельно размещен на Orion; все эти горяче-розовые квадраты карандашей [<img src="../images/edit.png" style="width:20px">](https://cms.sunstarsys.com/redirect?uri=https://www.sunstarsys.com/index) Справа сверху живут.  Не стесняйтесь крутиться и экспериментировать со всем, что вы видите!  Единственное, что вы не можете сделать, это зафиксировать и наблюдать, как ваши изменения строятся и развертываются; это ограничено нашим персоналом по очевидным причинам.

Наша глобальная сеть Point of Presence (POP) обеспечивает связь с Orion с низкой задержкой &trade; и веб-сайты конечной доставки, которые мы размещаем.  В настоящее время мы находимся на четырех континентах и имеем бикостальное присутствие в США.  Мы стремительно развиваемся в Африке и Восточной Азии!

<h3>Пришло время перемен в том, как вы управляете и доставляете свой сайт целевой аудитории.</h3>

Идите в своем собственном темпе, используйте столько сотрудников / предварительный просмотр / правки / фиксации / филиалы, сколько вам нравится, и платите только за то, что вы *фактически используете* в нашей инфраструктуре. Просто возьмите с собой улучшенную платформу CSS / JavaScript, сценарии perl и шаблоны Django, и мы позаботимся об остальной части вашей настройки без дополнительной платы. Мы рекомендуем **Bootstrap 5** для стилей css/js, так как это то, что мы используем внутри; но это полностью зависит от вас.

Самый быстрый способ начать – играть с @SunStarSys/orion <https://iconoclasts.blog> и выдолбить источники в багажнике/контенте. ~90% настроек стиля в [`багажник/содержимое/css/local.css`](https://github.com/SunStarSys/www.iconoclasts.blog/blob/trunk/content/css/local.css) &mdash; Приспосабливайтесь к вкусу!

Кроме того, все клиенты имеют бесплатные, настраиваемые шаблонами или поддерживаемые JavaScript интерфейсы для нашей мощной, проприетарной поисковой системы PCRE.

Смотреть [Планы Ориона](plans) для текущих сделок.

</div>
</div>

### Сетка сравнения характеристик

<br />

| Особенность     |   Орион       | Понятие | Слияние |
|:^-----------|:-------------:|:------:|:----------:|
{% for row in grid.content %}| {{ row.Feature|safe }} |  <span class="text-success">{{ row.Orion|safe }}</span> | <span class="text-danger">{{ row.Notion|safe }}</span> |  <span class="text-danger">{{ row.Confluence|safe }}</span> |
{% endfor -%}

<br />

### Сведения

{% for d in deps %}
- [{{d.1.headers.title|safe}}]({{d.0}}) &mdash; {{d.1.content|lede}}
{% endfor %}

<hr />
<br />
<h2>Несколько репрезентативных снимков экрана:</h2>
<br />
<h3>IDE View с немецким, французским, ивритом и русским языком (UTF-8).</h3>
<center><img src="../images/ide-fa.png" style="width:800px"></center>
<br/>
<h3>Представление редактора IDE.</h3>
<center><img src="../images/ide-editor.png" style="width:800px"></center>
<br/>
<h3>Представление редактора IDE (на испанском языке).</h3>
<center><img src="../images/ide-es.png" style="width:800px"></center>
<br/>
<h3>Полная сборка сайта IDE.</h3>
<center><img src="../images/ide-build.png" style="width:800px"></center>
<br />
<h3>Типичная сборка сайта IDE.</h3>
<center><img src="../images/ide-file-build.png" style="width:800px"></center>
<br />
<h3><a href="https://cms.sunstarsys.com/redirect?uri=https://www.sunstarsys.com/;action=search;regex=mailto:">Поиск среды IDE</a> <small>(эти ссылки в <span style="color:#e83e8c">розовый</span> будет перетащить вас прямо на экран редактора, с курсором, размещенным точно в соответствии с соответствующим текстом)</small>.</h3>
<center><img src="../images/ide-search.png" style="width:800px"></center>
</div>

<style type="text/css">
/*-------------------------------------------
	Animations
-------------------------------------------*/
@-webkit-keyframes FADEY {
  0%   { opacity: 0; }
  100% { opacity: 1; }
}

.theme-showcase {
	-webkit-animation-name: FADEY;
	-webkit-animation-duration: 1s;
	-webkit-animation-timing-function: ease-in-out;
	-webkit-animation-iteration-count: 1;
}
</style>

<!-- $Date$ $Author$ $Revision$ -->
