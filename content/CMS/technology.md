...
- Subversion supports **fine-grained access control**, and lets you do partial/sparse checkouts of `HEAD`; with Git you have NO ACLs other than on an all-or-nothing branch push, and you must clone the entire branch (which includes history) prior.  If you don't recognize the necessity of these svn-only feature sets, you haven't groked the previous item's (see above) remarks quite yet.
