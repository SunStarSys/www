...
El &nbsp; <span class="text-white">SunStar Systems CMS</span> &nbsp; es un
<ul>
<li>en línea,</li>
...
El CMS hace todo esto interactuando con nuestro servicio Subversion habilitado para [svnpubsub](https://svn.apache.org/repos/asf/subversion/trunk/tools/server-side/svnpubsub/). El CMS construye automáticamente cada cambio confirmado utilizando un ultrarrápido [`node.js`](https://nodejs.org/) + [`Perl 7`](https://www.perl.com/article/anounce- perl-7/) sistema de compilación basado en el [Principio de Hollywood](https://deviq.com/hollywood-principle/): llama a los módulos Perl proporcionados por el cliente para hacer lo que el cliente necesite. (No se preocupe, hay plantillas de diseño básicas disponibles que harán todo el trabajo pesado por usted). También hace un esfuerzo adicional y distribuye instantáneamente los conjuntos de cambios para la compilación, **atómicamente**, a nuestro CDN perimetral. servidores web en nuestra red POP.

[Cumplimiento de REST](https://www.codecademy.com/articles/what-is-rest) tiene sus beneficios para los usuarios de CMS. Les gustará el hecho de que el botón <span class="text-white">Atrás</span> de su navegador no interrumpa el servicio; de hecho, es un componente esencial de la función <span class="text-white">Revertir</span>. En una situación de ruptura de sitio de emergencia, volver a la página de resultados del envío de <span class="text-white">Commit</span> incorrecto les brindará la capacidad de revertir la fusión <span class="text-white">que </span> cambio aplicado erróneo &mdash; con el clic de un botón &mdash; y reconstruir el sitio de nuevo a un buen estado conocido.

Actualmente proporcionamos un sistema de plantilla básico basado en <span class="text-white">Django</span> derivado de un [`Dotiac::DTL`](http://dotiac.sourceforge.net/cgi-bin/) personalizado index.pl) módulo de CPAN. Próximamente se brindará soporte para un sistema de compilación de Python 3. El [sistema de compilación CMS] con licencia Apache 2.0 (https://github.com/SunStarSys/cms), junto con una gran colección de módulos de soporte para aprovechar dentro de ese árbol, está disponible en GitHub.
