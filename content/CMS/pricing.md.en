...
- LIMITED TIME OFFER: <span class="text-success">$200 / month / managed site</span> (90% discount over WordPress/Netlify Enterprise).  First month is <span class="text-success">FREE</span>.
	...
	- Covers one <span class="text-white">SunStar Systems CMS</span>-managed public website. This is introductory pricing good through **September 7, 2020**.

	- <span class="text-success">Up to 10 GB</span> covering total SunStar Systems server filesystem storage.  Basically, this includes two source tree checkouts and one resultant build tree (also a checkout), and potentially the Subversion server's source repository if we are hosting it for you.  Includes `.svn` dirs (containing pristine copies of all your files), but not the results of site branch builds, nor the per user IDE checkouts, in calculating total size.  Additional storage available at <span class="text-success">$50 / GB / month</span>.
...
- Soup to nuts, pay-as-you-go plan: <span class="text-success">unlimited number of websites; $50 / GB / month storage space + $.20 / GB / month outbound transfer</span>. First 500 GB / month of outbound (and all inbound) is free. Includes automated SSL cert/key management via LetsEncrypt. Starting **August 1, 2020**.
