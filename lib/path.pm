  ...
  *dependencies = Load join "", <$deps>;
  ...
  walk_content_tree {
    ...
    if (/\.md[^\/]*$/) {
      ...
      push @{$dependencies{$path}}, grep $_ ne $path, grep s/^content//, map glob("content$_"),map index($_, "/") == 0  ? $_ : "$dir/$_", ref $d{headers}{dependencies} ? @{$d{headers}{dependencies}} : split /,?\s+/, $d{headers}{dependencies} if exists $d{headers}{dependencies};
    }
