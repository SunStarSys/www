...
{% ifequal specials "diff=" %}
  <div class="card text-white border-success mb-3">
    <div class="card-header">{{meta|vcs_author:r.path_info}}
      <small><em>
          på {{ meta|vcs_date:lang }}
          vid {{meta|vcs_time:lang}}
          </em></small></div>
    <div class="card-body">
    ...
    <dl>
      ...
      <dt><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=diff={{e.0}};lang={{lang}};markdown_search=1">{{e.0}}</a>
        {{e.3|vcs_author:r.path_info}} <small><em>
            på {{e.3|vcs_date:lang}}
            vid {{e.3|vcs_time:lang}}</em></small></dt>
        {% for k, v in e.1 %}
      <dd>
        <code>{{v.action}}</code>&nbsp;
    ...
    <div style="width:100%">
      <div class="right" id="filter">
        ...
        <input class="form-control" type="text" name="filter"
             placeholder="PCRE rekursiv sökning" value="{{ filter }}" />&nbsp;<button type="skicka"
    name="submit" value="1" class="btn btn-outline-danger"><i class="fa
    fa-filter fa-emoji" title="Filtrera"></i>
        </button>
      </div>

{% if matches.0 %}
		<pre>Totalt antal matchningar = {{ count }}, Totalt antal dokument = {{ file_count }}. Sorterad efter matchningsantal, sorterad efter dokumentrevision.</pre>
      <div>
        <dl>
          {% for m in matches %}
          <dt>{{ m.0|safe }}</dt>
