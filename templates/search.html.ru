...
<div class="container">
  ...
  <div class="card text-white border-success mb-3">
    <div class="card-header">{{meta|vcs_author:r.path_info}}
      <small><em>
        включено {{ meta|vcs_date:lang }}
          на {{meta|vcs_time:lang}}
          </em></small></div>
    <div class="card-body">
    ...
    <dl>
      ...
      <dt><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=diff={{e.0}};lang={{lang}};markdown_search=1">{{e.0}}</a>
        {{e.3|vcs_author:r.path_info}} <small><em>
            включено {{e.3|vcs_date:lang}}
            на {{e.3|vcs_time:lang}}</em></small></dt>
        {% for k, v in e.1 %}
      <dd>
        <code>{{v.action}}</code>&nbsp;
  ...
  <form class="form-inline row" method="POST">
    <div style="width:100%">
      <div class="right" id="filter">
        ...
        <input class="form-control" name="filter" placeholder="Рекурсивный поиск PCRE" type="text" value="{{ filter }}" />&nbsp;<button class="btn btn-outline-danger" name="submit" type="submit" value="1"><i class="fa
    fa-filter fa-emoji" title="Фильтр"></i>
        </button>
      </div>
      {% if matches.0 %}
		<pre>Всего соответствий = {{ count }}, Всего документов = {{ file_count }}. Заказано по числу совпадений; подчинено версии документа.</pre>
      <div>
        <dl>
          {% for m in matches %}
          <dt>{{ m.0|safe }}</dt>
